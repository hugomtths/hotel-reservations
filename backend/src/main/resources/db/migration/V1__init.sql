...
(
    ...
    CONSTRAINT unq_comodidade_nome
        UNIQUE (nome)
);
...
(
    ...
    CONSTRAINT unq_servico_adicional_nome
        UNIQUE (nome_servico),
...
CREATE TABLE users (
    ...
    CONSTRAINT chk_role_user
       CHECK (role IN ('CLIENTE', 'FUNCIONARIO'))
);
...
(
    ...
    CONSTRAINT chk_cargo_funcionario
        CHECK (cargo IN ('ATENDENTE', 'GERENTE'))
);
...
(
    ...
    CONSTRAINT chk_status_quarto
        CHECK (status IN ('DISPONIVEL', 'MANUTENCAO', 'LIMPEZA')),
    CONSTRAINT chk_area_quarto
        CHECK (area IS NULL OR area > 0),
    CONSTRAINT unq_quarto_hotel_numero
...
(
    ...
    CONSTRAINT fk_quarto_comodidade_quarto
        FOREIGN KEY (quarto_id) REFERENCES quarto (id) ON DELETE CASCADE,
    CONSTRAINT fk_quarto_comodidade_comodidade
...
(
    ...
    CONSTRAINT chk_status_reserva
        CHECK (status_reserva IN ('CONFIRMADA', 'CANCELADA', 'CONCLUIDA')),
    CONSTRAINT chk_datas_reserva
        CHECK (data_checkin_previsto < data_checkout_previsto)
);
...
(
    ...
    CONSTRAINT chk_valor_total_pagamento
        CHECK (valor_total >= 0),
